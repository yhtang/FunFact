...
class RBFExpansionMiniBatchPlus(RBFExpansionBase):
    ...
    def fit(
        ...
        with torch.random.fork_rng(devices=[self.device]):
            ...
            def loss_minibatch(rbf, target, i, j, u, v, a, b):
                return self.loss(
                    target[..., i, j],
                    torch.sum(
                        rbf(u[..., i, :] - v[..., j, :]) *
    ...
    #         self.report = self._grad_opt(
    #             target,
    #             self.ModelPlus(f, (u0, a0, b0), default_grad_on=True)
    #         )
    ...
    def fit_custom(self, target, f, f_minibatch, seed=None, plugins=[], **x0):
        ...
        with torch.random.fork_rng(devices=[self.device]):
            ...
            def loss_minibatch(rbf, target, i, j, *x):
                return self.loss(
                    target[..., i, j],
                    f_minibatch(rbf, i, j, *x),
                    reduction='mean'
                )
